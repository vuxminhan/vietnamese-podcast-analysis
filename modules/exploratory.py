...
# #spotify
df['description']=df['description'].astype(str)
df = pd.read_csv(PATH + "/data/spotify episodes_details_language_added.csv")
df.head()
df['release_date'] = pd.to_datetime(df['release_date'])
#remove podcast episode not in vietnamese
df = df[df['language'] == 'vi']

# # Group the data by year and concatenate tokenized names and descriptions for each year
# grouped_df = df.groupby(df['release_date'].dt.year).agg({'name': lambda x: ' '.join(map(tokenize_text, x))}).reset_index()
...
#     # Generate word cloud for video titles
#     title_wordcloud = WordCloud(width=800, height=400, background_color='white')
#     wordcloud = title_wordcloud.generate(row['name'])

#     # Get word frequencies
#     word_freq = title_wordcloud.process_text(row['name'])

#     # Convert word frequencies to a DataFrame
#     table_data = pd.DataFrame(list(word_freq.items()), columns=['Word', f"Year {int(row['release_date'])}"])
#     table_data.set_index('Word', inplace=True)

#     # Add the word frequencies to the dictionary
#     print(table_data.sort_values(by=f"Year {int(row['release_date'])}", ascending=False).head(10))

#     # Plot the word clouds
#     plt.figure(figsize=(8, 4))
#     plt.imshow(wordcloud, interpolation='bilinear')
#     plt.title(f"Most Common Words in Podcast Titles - Year {int(row['release_date'])}")
#     plt.axis('off')
...
grouped_df_description = df.groupby(df['release_date'].dt.year).agg({'description': lambda x: ' '.join(map(tokenize_text, x))}).reset_index()

# Create a dictionary to store the word frequencies for each year
word_freq_dict = {}

# Generate word clouds for video titles for each year and calculate word frequencies
for index, row in grouped_df_description.iterrows():
    # Generate word cloud for video titles
    title_wordcloud = WordCloud(width=800, height=400, background_color='white')
    wordcloud = title_wordcloud.generate(row['description'])

    # Get word frequencies
    word_freq = title_wordcloud.process_text(row['description'])
    ...
    plt.figure(figsize=(8, 4))
    plt.imshow(wordcloud, interpolation='bilinear')
    plt.title(f"Most Common Words in Podcast Description - Year {int(row['release_date'])}")
    plt.axis('off')
